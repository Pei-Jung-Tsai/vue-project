...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <input v-model="targetAuthor" placeholder="Please input author name:" class="author-input" />

      <ul>
        <li
          v-for="author in authors"
          :key="author.id"
          :title="isHighlight(author.name) ? `This is ${author.name}` : author.name"
          :class="{ highlight: isHighlight(author.name) }"
          :style="isHighlight(author.name) ? { color: highlightColor, fontWeight: 'bold' } : {}"
        >
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
...
const orwell = computed(() => authors.find((author) => author.name === 'George Orwell') || null)

// Activity 5: Find author by ID
const austen = computed(() => authors.find((author) => author.id === 1) || null)

const targetAuthor = ref('George Orwell')
const highlightColor = '#FFD700'

function isHighlight(name) {
  return name.toLowerCase() === targetAuthor.value.toLowerCase()
}
</script>
